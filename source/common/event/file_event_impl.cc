...
FileEventImpl::FileEventImpl(DispatcherImpl& dispatcher, os_fd_t fd, FileReadyCb cb,
  ...
  if (activate_fd_events_next_event_loop_) {
    activation_cb_ = dispatcher.createSchedulableCallback([this]() {
      ASSERT(pending_activation_events_ != 0);
      runCb(0);
    });
  }
}
...
void FileEventImpl::activate(uint32_t events) {
  ...
  if (activate_fd_events_next_event_loop_) {
    // Merge new events with pending synthetic events.
    if (pending_activation_events_ == 0) {
      // Schedule the activation callback so it runs as part of the next loop iteration
      ASSERT(!activation_cb_->enabled());
      activation_cb_->scheduleCallback(SchedulableCallback::ScheduleType::NextLoopIteration);
    }
    ASSERT(activation_cb_->enabled());
    pending_activation_events_ |= events;
    return;
  }
...
void FileEventImpl::setEnabled(uint32_t events) {
  if (activate_fd_events_next_event_loop_ && pending_activation_events_ != 0) {
    ...
    // will be able to determine the fd read/write state without need for the synthetic activation
    // events.
    pending_activation_events_ = 0;
...
void FileEventImpl::runCb(uint32_t events) {
  if (activate_fd_events_next_event_loop_ && pending_activation_events_ != 0) {
    events |= pending_activation_events_;
    pending_activation_events_ = 0;
    activation_cb_->cancel();
  }
  cb_(events);
