...
void DispatcherImpl::deferredDelete(DeferredDeletablePtr&& to_delete) {
  ...
  if (1 == current_to_delete_->size()) {
    deferred_delete_cb_->scheduleCallback(SchedulableCallback::ScheduleType::CurrentLoopIteration);
  }
}
...
void DispatcherImpl::post(std::function<void()> callback) {
  ...
  if (do_post) {
    post_cb_->scheduleCallback(SchedulableCallback::ScheduleType::CurrentLoopIteration);
  }
}
