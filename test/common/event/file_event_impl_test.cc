...
TEST_P(FileEventImplActivateTest, ActivateChaining) {
  ...
  // Note that the real and fake events are combined and delivered in a single call to the fd
  // callback.
  ...
  if (activate_fds_next_event_loop()) {
    ...
    // Recursive activation: handle write and close events scheduled while handling read.
    EXPECT_CALL(fd_event, ready());
    EXPECT_CALL(write_event, ready());
    EXPECT_CALL(closed_event, ready());
    // Second recursive activation: handle close event scheduled while handling write.
    EXPECT_CALL(fd_event, ready());
    EXPECT_CALL(closed_event, ready());
...
TEST_P(FileEventImplActivateTest, SetEnableCancelsActivate) {
  ...
  // fake event is discarded by the setEnable call.
  EXPECT_CALL(prepare_watcher, ready());
  EXPECT_CALL(fd_event, ready());
  EXPECT_CALL(write_event, ready());
